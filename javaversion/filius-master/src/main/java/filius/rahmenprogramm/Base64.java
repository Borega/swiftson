...
public class Base64 {
    ...
    /*
     * Rechner platform me be something funny like EBCDIC, so we hardcode these values.
     */
    ...
    private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
        ...
        // Example: DkLE
            ...
            try {
                ...
                LOG.debug("EXCEPTION (static), Base64.java):");
    ...
    public static byte[] decode(byte[] source, int off, int len, int options) {
        ...
        for (i = off; i < off + len; i++) {
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) // White space, Equals sign or better
            {
                ...
                LOG.debug("Invalid Base64 input character at " + i + ": decimal=" + source[i] + " / char="
                        + (char) source[i]);
                return null;
            } // end else:
    ...
    public static byte[] decode(String s) {
        byte[] decodedBytes = decode(s, NO_OPTIONS);
    ...
    public static byte[] decodeFromFile(String filename) {
        ...
        try {
            ...
            LOG.debug("EXCEPTION (static): Error decoding from file " + filename);
        } // end catch: IOException
        finally {
            try {
                bis.close();
    ...
    public static String encodeFromFile(String filename) {
        ...
        try {
            ...
            // LOG.debug("DEBUG, Base64, encodeFromFile: bis=" +
            // (bis==null ? "<null>" : bis) );
            ...
            // LOG.debug("DEBUG, Base64, encodeFromFile: length=" +
            // length );

            // Save in a variable to return
            encodedData = new String(buffer, 0, length, Base64.PREFERRED_ENCODING);

            // LOG.debug("DEBUG, Base64, encodeFromFile: encodedDate="
            // + encodedData );
